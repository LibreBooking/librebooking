                ...
                {if !$hideDetails}
                    {if $attributes|default:array()|count > 0}
                        ...
                        {foreach from=$attributes item=attribute}
                            ...
                            {capture name="attr"}
                                <div>
                                    <span class="fw-bold">{$attribute->Label()|escape:'html'}:</span>
                                    {if $attribute->Type() == 4}
                                        {if $attribute->Value() == '1'}{translate key='True'}{else}{translate key='False'}{/if}
                                    {elseif $attribute->Type() == 5}
                                        {formatdate date=$attribute->Value() key=general_datetime}
                                    {elseif $attribute->Type() == 2}
                                        {$attribute->Value()|escape:'html'|nl2br}
                                    {else}
                                        {$attribute->Value()|escape:'html'}
                                    {/if}
                                </div>
                            {/capture}
                            {$smarty.capture.attr}
